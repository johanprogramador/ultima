...
from django.utils.timezone import now  # Añade esto con los otros imports
...
class RolUser(AbstractUser):
    ...
    def clean(self):
        if self.email:
            ...
            if RolUser.objects.filter(email=self.email).exclude(pk=self.pk).exists():
                raise ValidationError({'email': 'Este correo electrónico ya está en uso'})
        
        if self.documento and RolUser.objects.filter(documento=self.documento).exclude(pk=self.pk).exists():
            raise ValidationError({'documento': 'Este documento ya está registrado'})
...
class Servicios(models.Model):
    ...
    def clean(self):
        if self.codigo_analitico and Servicios.objects.filter(codigo_analitico=self.codigo_analitico).exclude(pk=self.pk).exists():
            raise ValidationError({'codigo_analitico': 'Este código analítico ya está en uso'})
...
class Dispositivo(models.Model):
    ...
    placa_cu = models.CharField(max_length=50, null=True, blank=True)
    ...
    def clean(self):
        ...
        if Dispositivo.objects.filter(serial=self.serial).exclude(pk=self.pk).exists():
            raise ValidationError({'serial': 'Este número de serie ya está registrado'})
            
        if self.placa_cu and Dispositivo.objects.filter(placa_cu=self.placa_cu).exclude(pk=self.pk).exists():
            raise ValidationError({'placa_cu': 'Esta placa CU ya está registrada'})
...
class Movimiento(models.Model):
    ...
    encargado = models.ForeignKey('RolUser', on_delete=models.SET_NULL, null=True, blank=True, related_name='movimientos_realizados')
    fecha_movimiento = models.DateTimeField(auto_now_add=True)
    posicion_origen = models.ForeignKey('Posicion', on_delete=models.SET_NULL, null=True, blank=True, related_name='movimientos_salida')
    posicion_destino = models.ForeignKey('Posicion', on_delete=models.SET_NULL, null=True, blank=True, related_name='movimientos_entrada')
    ubicacion_origen = models.CharField(max_length=20, choices=UBICACIONES, null=True, blank=True)
    ubicacion_destino = models.CharField(max_length=20, choices=UBICACIONES, null=True, blank=True)
    ...
    def generar_observacion(self):
        ...
        if self.encargado:
            partes.append(f"Realizado por: {self.encargado.nombre}")
...
class UsuarioExterno(models.Model):
    ...
    documento = models.CharField(max_length=20, validators=[MinLengthValidator(5)], help_text="Número de documento de identidad")
    ...
    def clean(self):
        if UsuarioExterno.objects.filter(documento=self.documento).exclude(pk=self.pk).exists():
            raise ValidationError({'documento': 'Este documento ya está registrado'})
...
class RegistroMovimientoDispositivo(models.Model):
    ...
    asignacion = models.ForeignKey(AsignacionDispositivo, on_delete=models.CASCADE, related_name='movimientos')
    ...
    def clean(self):
        ...
        if ultimo_movimiento and ultimo_movimiento.tipo == self.tipo:
            raise ValidationError(
...
def registrar_login(sender, request, user, **kwargs):
    ...
    hace_un_minuto = now() - timedelta(minutes=1)  # Usa now() importado
...
def registrar_eliminacion(sender, instance, **kwargs):
    if sender.__name__ in ['Dispositivo', 'Movimiento', 'RolUser', 'UsuarioExterno']:
        sede = None
