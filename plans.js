    ...
    const [debug, setDebug] = useState(false) // Estado para activar/desactivar el modo debug
    ...
    useEffect(() => {
        fetchSelectorData()
        fetchPositions()
    }, [selectedPiso]) // Volver a cargar cuando cambie el piso seleccionado
    ...
    const renderTableCell = (position, row, col, isSelected, isMainCell, colSpan, rowSpan) => {
        ...
        <td
            ...
            outline: position ? "2px solid rgba(0, 0, 0, 0.3)" : "none",
            boxShadow: position ? "inset 0 0 5px rgba(0, 0, 0, 0.2)" : "none",
            }}
            className={`table-cell ${isSelected ? "selected" : ""} ${isMainCell ? "main-cell" : ""} ${position ? "has-position" : ""}`}
            data-position-id={position?.id || ""}
            data-debug={debug ? JSON.stringify({
            id: position?.id,
            fila: position?.fila,
        ...
        >
            ...
            {debug && position && (
            <div className="position-debug" style={{ position: "absolute", bottom: 0, right: 0, fontSize: "8px", color: "#666" }}>
                {position.id}
            </div>
            )}
        </td>
    ...
    return (
        ...
        <div className="controls-container">
            ...
            {/* Botón para recargar posiciones */}
            <button 
                className="action-button" 
                onClick={() => fetchPositions()} 
                title="Recargar posiciones"
            ...
            <div className="view-mode-buttons" style={{ marginLeft: "10px" }}>
                <button
                className={`action-button ${viewMode === "plano" ? "active" : ""}`}
                ...
                <button
                className={`action-button ${viewMode === "tabla" ? "active" : ""}`}
